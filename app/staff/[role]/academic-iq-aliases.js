...
export default function StaffAliases() {
  ...
  // has no topicAliases base — and an undefined apiBase produces requests to "undefined/...".
  if (!portal?.topicAliases) return null;

  const tree = ALIAS_TREES.topicAliases;
  return (
    <AliasManagerScreen
      homeRoute={`/staff/${roleKey}`}
      apiBase={portal.topicAliases}
      title={tree.title}
      levels={tree.levels}
    />
  );
