...
export default function StaffAliases() {
  ...
  // has no codingProAliases base — and an undefined apiBase produces requests to "undefined/...".
  if (!portal?.codingProAliases) return null;

  const tree = ALIAS_TREES.codingProAliases;
  return (
    <AliasManagerScreen
      homeRoute={`/staff/${roleKey}`}
      apiBase={portal.codingProAliases}
      title={tree.title}
      levels={tree.levels}
    />
  );
